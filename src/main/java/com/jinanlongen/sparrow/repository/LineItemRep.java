...
public interface LineItemRep extends JpaRepository<LineItem, Long> {
  ...
  @Query(value = "SELECT COUNT(*) FROM LINE_ITEMS WHERE M_ID=? AND COLOR=? AND SIZE=? AND ID!=?",
      nativeQuery = true)
  int count(long mid, String color, String size, long id);

  @Query(value = "SELECT COUNT(*) FROM LINE_ITEMS WHERE M_ID=? AND COLOR=? AND SIZE=? ",
      nativeQuery = true)
  int count2(long mid, String color, String size);

  @Query(value = "SELECT COUNT(*) FROM LINE_ITEMS WHERE M_ID=? AND SALES_STATE='在售中' ",
      nativeQuery = true)
  int count3(long mid);

  @Query(
      value = "SELECT COUNT(*) FROM LINE_ITEM_ROC_IDS WHERE ROC_IDS=? AND LINE_ITEM_ID IN(SELECT ID FROM LINE_ITEMS WHERE M_ID IN (SELECT ID FROM MERCHANDISES WHERE STORE_ID IN(SELECT STORE_ID FROM MERCHANDISES WHERE ID=?)) ) ",
      nativeQuery = true)
  int countRocid(String rocid, long mid);

  @Query(
      value = "SELECT COUNT(*) FROM LINE_ITEM_ROC_IDS WHERE ROC_IDS=? AND LINE_ITEM_ID IN(SELECT ID FROM LINE_ITEMS WHERE M_ID IN (SELECT ID FROM MERCHANDISES WHERE STORE_ID IN(SELECT STORE_ID FROM MERCHANDISES WHERE ID=?)) AND  LINE_ITEM_ID!=?) ",
      nativeQuery = true)
